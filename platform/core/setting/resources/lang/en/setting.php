...
return [
    ...
    'cache' => [
        ...
        'form' => [
            ...
            'cache_time_site_map' => 'Sitemap cache timeout (in minutes)',
            'enable_public_cache_control' => 'Public cache headers (CDN / reverse proxy)',
            'enable_public_cache_control_helper' => 'Sends <code>Cache-Control: public</code> on guest page views and drops per-visitor cookies, so a CDN or reverse proxy (Cloudflare, Varnish, nginx) can serve them without hitting PHP. Admin pages, logged-in visitors and any page containing a CSRF token are always excluded. This only speeds up your site if you actually have a CDN or proxy in front of it - on its own it just lets browsers reuse the page for the duration below. With <code>APP_DEBUG</code> on, responses that are skipped carry an <code>X-Public-Cache-Skip</code> header explaining why.',
            'public_cache_max_age' => 'Public cache duration (seconds)',
            'public_cache_max_age_helper' => 'How long a CDN, proxy or browser may reuse a guest page before asking again. Default is 600 seconds (10 minutes). Higher values are faster but content edits take longer to appear.',
