...
class HandleApplyCouponService
{
    ...
    public function execute(string $coupon, array $sessionData = [], array $cartData = [], ?string $prefix = ''): array
    {
        ...
        if (Arr::get($resultCondition, 'error') != false) {
    ...
    public function checkConditionDiscount(Discount $discount, array $sessionData = [], ?int $customerId = 0): array
    {
    ...
    protected function getCouponDiscountAmount(Discount $discount, array $cartData = []): array
    {
        $couponDiscountAmount = 0;
