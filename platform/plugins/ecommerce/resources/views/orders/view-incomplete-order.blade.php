                    ...
                    <div class="ui-banner__content ws-nm">
                        ...
                        <div class="ws-nm">
                            ...
                            @if ($order->user->email || $order->address->email)
                                <button
                                    class="btn btn-secondary btn-trigger-send-order-recover-modal"
                                    data-action="{{ route('orders.send-order-recover-email', $order->id) }}"
                                >{{ trans('plugins/ecommerce::order.send_an_email_to_recover_this_order') }}</button>
                            @else
        ...
        <div class="flexbox-grid">
                                            ...
                                            @if ($product && $product->original_product)
                                                <tr>
                                                    ...
                                                    <td class="pl5 p-r5 width-30-px min-width-30-px text-start">
                                                        <span
                                                            class="item-quantity text-end">{{ $orderProduct->qty }}</span>
                                                    </td>
                        ...
                        <form action="{{ route('orders.edit', $order->id) }}">
                            ...
                            <div class="mt15 mb15 text-end">
                                <button
                                    ...
                                    type="button"
                                >{{ trans('plugins/ecommerce::order.save_note') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div class="flexbox-content flexbox-right">
                            ...
                            <li class="overflow-ellipsis">
                                <div class="mb5">
                                    <a
                                        ...
                                        href="#"
                                    >{{ $order->user->name ?: $order->address->name }}</a>
                                </div>
                                @if ($order->user->id)
                                    <div><i
                                ...
                                <ul class="ws-nm text-infor-subdued">
                                    <li class="overflow-ellipsis"><a
                                        ...
                                        >{{ $order->user->email ?: $order->address->email }}</a></li>
                                    @if ($order->user->id)
                                        <li>
                            ...
                            <li class="text-infor-subdued mt15">
                                ...
                                <div>
                                    ...
                                    <div>
                                        <a
                                            class="hover-underline"
                                            href="https://maps.google.com/?q={{ $order->full_address }}"
                                            target="_blank"
                                    ...
                                    @foreach (['ip', 'landing_domain', 'landing_page', 'landing_params', 'referral', 'gclid', 'fclid', 'utm_source', 'utm_campaign', 'utm_medium', 'utm_term', 'utm_content', 'referrer_url', 'referrer_domain'] as $field)
                                        @if ($order->referral->{$field})
                                            <li>{{ trans('plugins/ecommerce::order.referral_data.' . $field) }}: <strong
                                                ...
                                                >{{ $order->referral->{$field} }}</strong></li>
                                        @endif
                                    @endforeach
                                </ul>
                            </div>
                ...
                <div class="wrapper-content mb20">
                    <div class="pd-all-20">
                        <button
                            class="btn btn-warning btn-mark-order-as-completed-modal"
                            data-action="{{ route('orders.mark-as-completed', $order->id) }}"
                        >{{ trans('plugins/ecommerce::order.mark_as_completed.name') }}</button>&nbsp;
                    </div>
                </div>
            </div>
        </div>
