@foreach ($products as $product)
    <div class="list-group-item px-0 px-md-2">
        <input
            type="hidden"
            name="up_sale_products[{{ $product->id }}][id]"
            value="{{ $product->id }}"
        />
        <input
            type="hidden"
            name="up_sale_products[{{ $product->id }}][is_variant]"
            value="0"
        />
        ...
        <div class="row">
            <div class="col">
                <x-core::form.text-input
                    ...
                    name="up_sale_products[{{ $product->id }}][price]"
                />
            </div>
            <div class="col">
                <x-core::form.select
                    ...
                    name="up_sale_products[{{ $product->id }}][price_type]"
                />
            </div>
        </div>
        ...
        @if($product->variations->isNotEmpty())
            <x-core::form.on-off.checkbox
                ...
                name="up_sale_products[{{ $product->id }}][apply_to_all_variations]"
                :checked="$product->pivot->apply_to_all_variations"
                data-bb-toggle="collapse"
                data-bb-target="#up-sale-product-variations-{{ $product->id }}"
                data-bb-reverse
            />

            <div class="list-group" id="up-sale-product-variations-{{ $product->id }}" @style(['display: none' => $product->pivot->apply_to_all_variations])>
                @foreach($product->variations as $variationProduct)
                    @php($variationProduct->product->pivot = $upSaleProducts->find($variationProduct->product->id)?->pivot)

                    <input
                        type="hidden"
                        name="up_sale_products[{{ $variationProduct->product->id }}][id]"
                        value="{{ $variationProduct->product->id }}"
                    />
                    <input
                        type="hidden"
                        name="up_sale_products[{{ $variationProduct->product->id }}][parent_id]"
                        value="{{ $product->id }}"
                    />
                    <input
                        type="hidden"
                        name="up_sale_products[{{ $variationProduct->product->id }}][is_variant]"
                        value="1"
                    />
                        ...
                        <div class="row">
                            <div class="col">
                                <x-core::form.text-input
                                    ...
                                    name="up_sale_products[{{ $variationProduct->product->id }}][price]"
                                />
                            </div>
                            ...
                            <div class="col">
                                <x-core::form.select
                                    ...
                                    name="up_sale_products[{{ $variationProduct->product->id }}][price_type]"
                                />
                            </div>
                        </div>
                    </div>
