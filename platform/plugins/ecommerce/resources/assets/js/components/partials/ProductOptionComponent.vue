        ...
        <div v-for="option in options" v-bind:key="option.id">
            ...
            <div v-if="option.option_type == 'checkbox'">
                <div class="form-check" v-for="value in option.values" v-bind:key="value.id">
                    <input class="form-check-input" type="checkbox" :name="'option-' + option.id" @input="changeInput($event, option, value)" :value="value.option_value" :id="'form-check-' + value.id">
                    <label class="form-check-label" :for="'form-check-' + value.id">{{ value.title }}</label>
                </div>
            </div>
            <div v-if="option.option_type == 'radio'">
                <div class="form-check" v-for="value in option.values" v-bind:key="value.id">
                    <input class="form-check-input" type="radio" :name="'option-' + option.id" @input="changeInput($event, option, value)" :value="value.option_value" :id="'form-check-' + value.id">
                    <label class="form-check-label" :for="'form-check-' + value.id">{{ value.title }}</label>
                </div>
            </div>
            <div v-if="option.option_type == 'field'">
                <div class="form-floating mb-3" v-for="value in option.values" v-bind:key="value.id">
                    <input type="text" class="form-control" @input="changeInput($event, option, value)" :name="'option-' + option.id" :id="'form-input-' + value.id" placeholder="...">
