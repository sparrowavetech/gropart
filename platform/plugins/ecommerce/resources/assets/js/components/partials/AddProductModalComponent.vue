<template>
    <div>
        <b-modal id="add-product-item" :title="__('order.add_product')" :ok-title="__('order.save')" :cancel-title="__('order.cancel')"
            ...
            <div class="form-group mb-3" v-if="store && store.id">
                <label class="next-label">{{ __('order.store') }}: <strong class="text-primary">{{ store.name }}</strong></label>
            </div>
        </b-modal>
    </div>
</template>
