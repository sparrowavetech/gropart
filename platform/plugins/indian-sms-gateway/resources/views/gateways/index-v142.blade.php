...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const providerHints = {
        ...
        fast2sms: 'Fast2SMS DLT route requires a mapped provider Message ID. Direct routes can send message text.',
    ...
    function refresh() {
        ...
        const isFast2SmsDlt = gateway.value === 'fast2sms' && route === 'dlt';
        hint.textContent = providerHints[gateway.value] || 'Select an approved template only when required by the provider route.';

        if (isFast2SmsDlt && (!option || !option.value)) {
            status.innerHTML = '<span class="badge bg-danger-lt">Template required for Fast2SMS DLT</span>';
