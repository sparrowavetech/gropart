@php
    /** @var int $productId */
    $bundles = $bundles ?? product_bundles_for_product($productId);
...
@if ($bundles->count())
<div class="pb-wrap" data-product-id="{{ $productId }}" data-add-url="{{ route('product-bundles.add_to_cart') }}">
    <div class="pb-title">{{ trans('plugins/product-bundles::bundles.front.title') }}</div>

    @foreach ($bundles as $bundle)
        <div class="pb-card" data-bundle-id="{{ $bundle->id }}" data-bundle-type="{{ $bundle->type }}">
            <div class="pb-card__head">
                <div class="pb-name">{{ $bundle->name }}</div>
                @if ($bundle->description)
                    <div class="pb-desc">{{ $bundle->description }}</div>
                @endif
            </div>
