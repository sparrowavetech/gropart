...
<table class="line-items-container">
    ...
    <tbody>
        ...
        @if (EcommerceHelper::isTaxEnabled())
            @php
                $taxComponents = [];
                // Aggregate tax components from all items
                foreach ($invoice->items as $item) {
                    if ($item->taxComponents) {
                        foreach ($item->taxComponents as $component) {
                             $key = $component->code . '-' . $component->rate;
                             if (!isset($taxComponents[$key])) {
                                 $taxComponents[$key] = [
                                     'name' => $component->name . ' (' . (float)$component->rate . '%)',
                                     'amount' => 0
                                 ];
                             }
                             $taxComponents[$key]['amount'] += $component->amount;
                        }
                    }
                }
            @endphp
            ...
            @if (!empty($taxComponents))
                @foreach ($taxComponents as $tax)
                    <tr>
                        ...
                        <td class="bold">
                            {!! htmlentities(format_price($tax['amount'])) !!}
                        </td>
                    </tr>
                @endforeach
            @else
