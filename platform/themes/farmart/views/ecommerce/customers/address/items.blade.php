    ...
    @forelse ($addresses as $address)
        <div class="col">
            <address class="border rounded p-2">
                <p>{{ $address->name }}<br> {{ $address->address }}<br> {{ $address->city_name }}<br>
                    {{ $address->state_name }}
                ...
                <div class="d-flex justify-content-between">
                    <div>
                        ...
                        >{{ __('Edit') }}</a>
                        <a
                            class="text-danger btn-trigger-delete-address ms-2"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button
                    class="btn btn-secondary border-0 py-2"
                    data-bs-dismiss="modal"
                    type="button"
                >{{ __('Cancel') }}</button>
                <button
                    class="btn btn-primary py-2 mb-0 avatar-save btn-confirm-delete"
                    type="button"
                >{{ __('Delete') }}</button>
            </div>
        </div>
