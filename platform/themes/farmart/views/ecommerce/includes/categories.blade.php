...
<ul @class(['loading-skeleton'])>
    ...
    @if($currentCategories)
        @forelse ($currentCategories as $category)
            @if (!empty($categoriesRequest) && $loop->first && !$category->parent_id)
                <li class="category-filter  mb-2">
                    <a class="nav-list__item-link" data-id=""
            ...
            <li @class([ 'category-filter' , 'opened'=> in_array($category->id, $categoriesRequest) && ($activeCategoryId == $category->id || $urlCurrent != route('public.single', $category->url)),])>
                <div class="widget-layered-nav-list__item">
                    <div class="nav-list__item-title">
                        <a data-id="{{ $category->id }}"
                            href="{{ route('public.single', $category->url) }}{{ $is_enquiry == 1?'?enquiry=1':''}}"
                            @class(['nav-list__item-link' , 'active'=>$activeCategoryId == $category->id || $urlCurrent == route('public.single', $category->url),])>
        ...
        @empty
            <li class="category-filter mb-2">
                <a class="nav-list__item-link" data-id="" href="{{ $is_enquiry == 1 ? route('public.product.enquiry'):route('public.products') }}{{ $is_enquiry == 1?'?enquiry=1':''}}">
                    <span class="cat-menu-close svg-icon">
                        <svg><use href="#svg-icon-chevron-left" xlink:href="#svg-icon-close"></use></svg>
                    </span>
                    <span>{{ __('All categories') }}</span>
                </a>
            </li>
