...
<div class="form-group">
    ...
    <div class="ui-select-wrapper form-group">
        <select name="is_infinite" class="ui-select">
            <option value="no"
                    @if (Arr::get($attributes, 'is_infinite', $defaultAutoplay ?? 'no') == 'no') selected @endif>{{ trans('core/base::base.no') }}</option>
            <option value="yes"
                    @if (Arr::get($attributes, 'is_infinite', $defaultAutoplay ?? 'no') == 'yes') selected @endif>{{ trans('core/base::base.yes') }}</option>
        </select>
        <svg class="svg-next-icon svg-next-icon-size-16">
            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#select-chevron"></use>
        </svg>
