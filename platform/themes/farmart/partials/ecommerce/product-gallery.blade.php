            ...
            <div class="product-gallery__image item">
                <a class="img-fluid-eq" href="{{ RvMedia::getImageUrl($img) }}">
                    ...
                    <div class="img-fluid-eq__wrap">
                        <img class="mx-auto" alt="{{ $product->name }}" src="{{ image_placeholder($img) }}" data-lazy="{{ RvMedia::getImageUrl($img) }}">
                    </div>
        ...
        @empty
            <div class="product-gallery__image item">
                <a class="img-fluid-eq" href="{{ image_placeholder() }}">
                    <div class="img-fluid-eq__dummy"></div>
                    <div class="img-fluid-eq__wrap">
                        <img class="mx-auto" alt="{{ $product->name }}" src="{{ image_placeholder() }}">
                    </div>
                </a>
            </div>
        @endforelse
        ...
        @forelse ($productImages as $img)
            <div class="item">
                <div class="border p-1 m-1">
                    <img class="lazyload" alt="{{ $product->name }}" src="{{ image_placeholder($img, 'thumb') }}" data-src="{{ RvMedia::getImageUrl($img, 'thumb') }}">
                </div>
            </div>
        @empty
            <div class="item">
                <div class="border p-1 m-1">
                    <img alt="{{ $product->name }}" src="{{ image_placeholder() }}">
                </div>
            </div>
        @endforelse
    </div>
