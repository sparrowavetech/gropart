        ...
        <div class="card-body p-4">
            <div class="customer-order-detail">
                <div class="row">
                    <div @class(['col-12' => ! $order->address->name, 'col-md-6' => $order->address->name])>
                        ...
                        <p>
                            ...
                            <strong class="text-info">{!! BaseHelper::clean($order->status->toHtml()) !!}</strong>
                        </p>
                        ...
                        @if (is_plugin_active('payment') && $order->payment->id)
                            ...
                            <p>
                                ...
                                <strong class="text-info">{!! BaseHelper::clean($order->payment->status->toHtml()) !!}</strong>
                            </p>
                        @endif
                    ...
                    @if ($order->address->name)
                        <div class="col-md-6">
                            ...
                            <p>
                                <span class="d-inline-block me-1">{{ __('Contact Number') }}: </span>
                ...
                <div>
                        ...
                        <table class="table table-striped table-hover align-middle">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center" style="width: 80px">{{ __('Image') }}</th>
                                    <th>{{ __('Product') }}</th>
                                    <th class="text-center" style="width: 140px">{{ __('Amount') }}</th>
                                    <th class="text-center" style="width: 100px">{{ __('Quantity') }}</th>
                                    <th class="price text-end" style="width: 140px">{{ __('Total') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <img src="{{ RvMedia::getImageUrl($orderProduct->product_image, 'thumb', false, RvMedia::getDefaultImage()) }}"
                                                alt="{{ $orderProduct->product_name }}" width="50" class="rounded border">
                                        </td>
                                        <td>
                                            @if ($product && $product->original_product?->url)
                                                <a href="{{ $product->original_product->url }}" class="fw-medium">{!! BaseHelper::clean($orderProduct->product_name) !!}</a>
                                            @else
                                                <span class="fw-medium">{!! BaseHelper::clean($orderProduct->product_name) !!}</span>
                                            @endif
                                            ...
                                            @if (is_plugin_active('marketplace') && ($product = $orderProduct->product) && $product->original_product->store?->id)
                                                <p class="d-block mb-0 sold-by mt-1">
                    ...
                    <div class="mt-3">
                        ...
                        @if ((float)$order->shipping_amount && EcommerceHelper::countDigitalProducts($order->products) != $order->products->count())
                            <p class="mb-1">
                                <span class="d-inline-block me-1">{{ __('Delivery Charge') }}: </span>
                ...
                @if ($order->shipment->id)
                    <hr class="my-4">
                    <h5 class="mb-3">{{ __('Delivery Information:') }}</h5>
                    <p class="mb-1">
                        <span class="d-inline-block me-1">{{ __('Delivery Status') }}: </span>
                    ...
                    @if ($order->shipment->shipping_company_name)
                        <p class="mb-1">
                            <span class="d-inline-block me-1">{{ __('Delivery Partner Name') }}: </span>
                        ...
                        <p class="mb-1">
                            ...
                            <strong class="d-inline-block">
                                <a href="{{ $order->shipment->tracking_link }}"
                                    target="_blank" class="text-primary text-decoration-underline">{{ $order->shipment->tracking_link }}</a>
                            </strong>
                        </p>
                    @endif
...
@elseif (request()->input('order_id') || request()->input('email') || request()->input('phone'))
